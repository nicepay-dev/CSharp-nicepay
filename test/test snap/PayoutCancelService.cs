...
public class PayoutCancelService
{
    ...
    public async Task cancelPayout_Test()
    {
        ...
        Console.WriteLine("Cancel Payout Response: " + jsonPayload);
        string externalId = SignatureGeneratorUtils.GenerateRandomNumberString(6);
        string createResponse = await payoutService.SendPostRequest(apiEndpoints.CancelPayout, accessToken, timestamp, bodyBuilder, externalId);
