...
public class PayoutRejectService
{
    ...
    public async Task rejectPayout_Test()
    {
       ...
       var bodyBuilder = new PayoutBuilder()
        ...
        Console.WriteLine("Reject Payout Response: " + jsonPayload);
        string externalId = SignatureGeneratorUtils.GenerateRandomNumberString(6);
        string createResponse = await payoutService.SendPostRequest(apiEndpoints.RejectPayout, accessToken, timestamp, bodyBuilder, externalId);
